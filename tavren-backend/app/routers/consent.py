...
async def log_consent_event(event: ConsentEventCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        log.error(f"Failed to log consent event: {e}", exc_info=True)
        await db.rollback()
        raise HTTPException(status_code=500, detail="Internal server error logging consent event.")
...
async def export_agent_training_log(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        log.error(f"Failed to export agent training log: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error exporting agent training log.")
...
async def get_reason_stats(db: AsyncSession = Depends(get_db), current_user: UserDisplay = Depends(get_current_active_user)):
    ...
    try:
        ...
        log.error(f"Failed to fetch reason stats: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error fetching reason stats.")
...
async def get_suggestion_success_stats(db: AsyncSession = Depends(get_db), current_user: UserDisplay = Depends(get_current_active_user)):
    ...
    try:
        ...
        suggestion_triggers = ["privacy", "trust", "complexity"]
        ...
        suggestions_accepted_query = select(func.count(ConsentEvent.id)).filter(
            ...
            ConsentEvent.reason_category == "suggested_alternative" # Assuming this category marks accepted suggestions
        ...
        log.error(f"Failed to fetch suggestion success stats: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error fetching suggestion success stats.") 